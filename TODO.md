...
- [~] **Firebase App Check.** Le chiavi Firebase nel client sono pubbliche per design: chiunque può scriptare richieste al backend Firebase FUORI dall'app (signup di massa, accesso Firestore entro le rules, generazione costi). Le Security Rules proteggono i *dati* ma non l'*abuso*.
  - [x] **Codice client (mobile) fatto** — `firebase_app_check` aggiunto + `FirebaseAppCheck.instance.activate()` in `main.dart` (Play Integrity su Android, App Attest+DeviceCheck su iOS, provider `debug` in debug build). Non blocca nulla finché l'enforcement è OFF: invia solo i token per il monitoraggio. Errore di attivazione non fatale.
  - [ ] **Console Firebase (lo devi fare tu):**
    1. Firebase Console → App Check → registra le app: **Android** con Play Integrity (serve l'app su Play Console + SHA-256 in Firebase), **iOS** con App Attest/DeviceCheck.
    2. Lancia una build **debug** sul device/emulatore, copia il token di debug stampato in console (`App Check debug token: ...`) e incollalo in App Check → app → "Manage debug tokens".
    3. Tieni l'enforcement **OFF** (monitor) per qualche giorno e guarda la dashboard: deve mostrare che il traffico legittimo invia token verificati.
    4. Solo quando la % di richieste verificate è alta → attiva **enforce** su Auth, Firestore, Storage. Un enforce affrettato blocca TUTTI gli utenti.
  - [ ] **Admin (web)** — manca: serve una `ReCaptchaV3Provider('<site-key>')` con la site key reCAPTCHA v3 generata in console. Aggiungere `firebase_app_check` al pacchetto admin e attivare con quella key una volta ottenuta.
